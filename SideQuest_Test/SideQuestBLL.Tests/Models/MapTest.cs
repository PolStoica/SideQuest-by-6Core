...
namespace SideQuest_Test.SideQuestBLL.Tests.Models
{
    public class MapTest
    {
        ...
        [Theory]
        [InlineData(Zone.Manastur)] // Avem evenimente in lista, dar niciunul in Manastur
        [InlineData(Zone.Centru)] // Lista de evenimente este complet goală (Empty List)
        [Trait("Feature", "MapFiltering")]
        [Trait("Type", "LogicTest")]
        [Trait("Priority", "P2")]
        ...
        [Theory]
        [InlineData(90.0, 180.0, true)]    // Valid (Marginea globului)
        [InlineData(-90.0, -180.0, true)]  // Valid (Marginea globului)
        [InlineData(91.0, 23.5, false)]    // Invalid (Peste limită)
        [InlineData(-91.0, 23.5, false)]   // Invalid (Peste limită)
        [InlineData(46.7, 181.0, false)]   // Invalid (Peste limită)
        [InlineData(0.0, 0.0, false)]      // Invalid (Regulă de business: Null Island)
        ...
        // Validare Distanță Maximă (Proximity Validation)
        // In cazul in care ne folosim doar de zona metropolitana a Clujului

        [Theory]
        [InlineData(46.7712, 23.5892, true)]  // Centru (Piața Unirii) - VALID
        [InlineData(46.7550, 23.5600, true)]  // Mănăștur - VALID
        [InlineData(46.5700, 23.7800, false)] // Turda - INVALID (prea departe)
        [InlineData(47.1400, 23.8700, false)] // Dej - INVALID (prea departe)
        [Trait("Feature", "MapLogic")]
        ...
        {
            // Arrange: User is near the boundary of Manastur and Center (Calea Manastur area)
            double userLat = 46.7610;
            double userLon = 23.5710;
            double searchRadius = 1.0; // 1 Kilometer

            // Event 1: Inside Manastur, 200m away
            var eventManastur = new Event { Title = "Manastur Coffee Shop", Lat = 46.7600, Lon = 23.5690 };

            // Event 2: Inside Center, 300m away
            var eventCenter = new Event { Title = "Center Art Gallery", Lat = 46.7630, Lon = 23.5730 };

            // Event 3: Too far away (Marasti), ~4km away
        ...
        {
            // Arrange: Latitude 46.76 is the LOWER limit for Centru
            // In your code: if (lat >= 46.76 ... return Zone.Centru;
        ...
        {
            // Arrange: Center coordinates with 18+ decimals
            // This is precision at a molecular level, impossible for real GPS
            ...
            // The system should not crash and should round the value to a manageable double
            result.Should().Be(Zone.Centru, "because the system should handle high-precision floats by rounding them naturally");
        }


        [Theory]
        [InlineData("46.77", "abc")]              // Text în loc de număr
        [InlineData("SELECT * FROM Users", "23")] // Tentativă SQL Injection
        [InlineData("46,77", "23.58")]            // Format greșit (virgulă în loc de punct)
        [InlineData("", " ")]                     // Câmpuri goale
        ...
        {
            // Arrange: Two different events at the exact same location (e.g., Cluj Arena)
